...
def cpu_utilization():
    thre_shold= int(input("Enter your thrash hold"))# take user input of the trshhold to compare with cpu usages
    current_cpu= psutil.cpu_percent(interval=1)   # to get Current cpu utilzation syntax of get psutil lbrary

    if current_cpu > thre_shold:  #use conditional statement for compare the condition  
        print("Cpu alert mail has been send to user")
    else :
